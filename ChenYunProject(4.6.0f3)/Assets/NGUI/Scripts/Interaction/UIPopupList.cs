...
public class UIPopupList : UIWidgetContainer
{
    ...
    public string contentBackgroundSprite;
    /// <summary>
    ...
    public void Show()
    {
        if (enabled && NGUITools.GetActive(gameObject) && mChild == null && atlas != null && isValid && items.Count > 0)
        {
            ...
            for (int i = 0, imax = items.Count; i < imax; ++i)
            {
                ...
                //modify
                //Add a sprite for the Content background
                mContentBackground = NGUITools.AddSprite(lbl.gameObject, atlas, contentBackgroundSprite);
                mContentBackground.pivot = UIWidget.Pivot.TopLeft;
                mContentBackground.depth = mHighlight.depth - 1;
                mContentBackground.transform.localPosition = Vector3.zero;
            ...
            for (int i = 0, imax = labels.Count; i < imax; ++i)
            {
                ...
                //modify
                UISprite sprite = lbl.GetComponentInChildren<UISprite>();
                sprite.width = (int)bcSize.x;
                sprite.height = (int)bcSize.y;
            }
